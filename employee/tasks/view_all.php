...
$actions = true;
$attributes = ["Task ID","Name", "Date Started", "Deadline", "Proj. No."];

$stmt = $pdo->prepare("SELECT tnumber,emp_task.STATUS,title,date_start,deadline,task.pnum 
                    FROM task JOIN emp_task ON tnum = tnumber JOIN dept_proj ON dept_proj.pnum = task.pnum 
                    WHERE dnum = :dept AND e_id = :e_id ORDER BY date_start DESC");
...
if ($search) {
    $stmt = $pdo->prepare("SELECT tnumber,emp_task.STATUS,title,date_start,deadline,task.pnum 
                            FROM task JOIN emp_task ON tnum = tnumber JOIN dept_proj ON dept_proj.pnum = task.pnum 
                            WHERE dnum = :dept AND e_id = :e_id AND title LIKE :search ORDER BY date_start DESC");
    $stmt->bindValue(":dept", $_SESSION['dept']);
    $stmt->bindValue(":e_id", $_SESSION['user_id']);
