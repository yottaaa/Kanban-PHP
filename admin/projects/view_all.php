...
$stmt = $pdo->prepare("SELECT pnum,STATUS,title,description,date_start,deadline,dnum FROM project 
                    JOIN dept_proj ON pnum = pnumber");
$stmt->execute();
...
if ($search) {
    $stmt = $pdo->prepare("SELECT pnum,STATUS,title,description,date_start,deadline,dnum FROM project 
                            JOIN dept_proj ON pnum = pnumber WHERE dept_proj.dnum LIKE :search or title LIKE :search");
...
<div class="mb-4 mt-5">
    <form>
        <div class="input-group">
            <input type="text" class="form-control" placeholder="Search Department id or Project name" name="search">
